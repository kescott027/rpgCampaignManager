...
from src.backend.controller_security import get_google_drive_key, get_google_service_account, load_config, get_google_oauth_creds
...
def get_drive_service_with_oauth(token: str):
    ...
    return build('drive', 'v3', credentials=None,
                 developerKey=None,
                 requestBuilder=lambda *args, **kwargs: requests.Request(*args, headers={
...
def search_google_drive(query: str, oauth_token=None, max_results=5):
    ...
    if not response.ok:
        raise RuntimeError(f"Google Drive API search failed: {response.status_code} - {response.text}")
...
def read_text_file(file_id: str, oauth_token=None) -> str:
    ...
    if response.status_code != 200:
        raise RuntimeError(f"Google Drive file read error: {response.status_code} - {response.text}")
    return response.text


def get_oauth_flow():
    client_secrets_path = Path(project_root()) / ".security" / "oauth_client_secret.json"
...
async def handle_google_oauth_callback(request: Request):
    try:
        ...
        if not code:
            return HTMLResponse(content="❌ No code provided in callback URL.", status_code=400)
